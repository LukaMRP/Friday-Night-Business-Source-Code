...
class TitleState extends MusicBeatState
{
	...
	var titleTextColors:Array<FlxColor> = [0xFFFF0000, 0xFF750000];
	...
	var easterEggKeys:Array<String> = [
		'SHADOW', 'RIVER', 'SHUBS', 'BBPANZU', 'ANNY'
	];
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('business', 'annyperez');
	...
	function startIntro()
	{
		...
		var background = new FlxSprite().loadGraphic(Paths.image('title/titleBG'));
		...
		switch(easterEgg.toUpperCase())
		{
			...
			case 'ANNY':
				gfDance.frames = Paths.getSparrowAtlas('AnnyBump');
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.camera.flash(ClientPrefs.flashing ? FlxColor.WHITE : 0xFFFF0101, 1);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Business'); // credTextShit.text += '\nFunkin';
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'ANNY':
						sound = FlxG.sound.play(Paths.sound('JingleAnny'));
